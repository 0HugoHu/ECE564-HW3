...
class DownloadVC: UIViewController, URLSessionDownloadDelegate {
    ...
    @IBAction func continueBtn(_ sender: Any) {
        if continueBtn.isEnabled && !self.errorFlag {
            let pvc = ViewController()
            pvc.dataModel = dataModel
            self.present(pvc, animated: true, completion: nil)
        } else {
            self.statusLabel.text = "You must do one operation successfully before continuing!"
        }
    }
    
    
    @IBOutlet weak var uploadHelp: UIImageView!
    @IBOutlet weak var downloadHelp: UIImageView!
    @IBOutlet weak var othersHelp: UIImageView!
    ...
    var errorFlag: Bool = true {
        didSet {
            continueBtn.isEnabled = dataModel.people.count > 0 && !errorFlag
        }
    }
    var errorMessage = ""
    ...
    override func viewDidLoad() {
        ...
        let tapGestureRecognizer1 = UITapGestureRecognizer(target: self, action: #selector(uploadHelpTapped))
        uploadHelp.addGestureRecognizer(tapGestureRecognizer1)
        uploadHelp.isUserInteractionEnabled = true
        
        let tapGestureRecognizer2 = UITapGestureRecognizer(target: self, action: #selector(downloadHelpTapped))
        downloadHelp.addGestureRecognizer(tapGestureRecognizer2)
        downloadHelp.isUserInteractionEnabled = true
        
        let tapGestureRecognizer3 = UITapGestureRecognizer(target: self, action: #selector(othersHelpTapped))
        othersHelp.addGestureRecognizer(tapGestureRecognizer3)
        othersHelp.isUserInteractionEnabled = true
        
    }
    
    
    @objc func uploadHelpTapped() {
        self.showToast(message: uploadHelpText, font: .systemFont(ofSize: 14.0))
    }
    
    
    @objc func downloadHelpTapped() {
        self.showToast(message: downloadHelpText, font: .systemFont(ofSize: 14.0))
    }
    
    
    @objc func othersHelpTapped() {
        self.showToast(message: othersHelpText, font: .systemFont(ofSize: 14.0))
    }
